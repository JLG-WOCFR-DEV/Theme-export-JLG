document.addEventListener('DOMContentLoaded', function() {
    ...
    (function initializeQuickActionsPanels() {
        const containers = document.querySelectorAll('[data-quick-actions]');

        if (!containers.length) {
            return;
        }

        const storageKey = 'tejlg:admin:quick-actions:dismissed';

        const readDismissedPreference = function() {
            try {
        ...
        const persistDismissedPreference = function(state) {
            try {
                window.localStorage.setItem(storageKey, state ? '1' : '0');
            } catch (error) {
                // Ignore storage errors so that the UI keeps responding.
            }
        };

        const isElementFocusable = function(element) {
        ...
        containers.forEach(function(container) {
            ...
            const menu = container.querySelector('[data-quick-actions-menu]');
            const dismiss = container.querySelector('[data-quick-actions-dismiss]');
            const restore = container.querySelector('[data-quick-actions-restore]');
            const actionSelector = '[data-quick-actions-link]';
            ...
            const setOpenState = function(state, options) {
            ...
            const setDismissedState = function(state, options) {
                const nextState = !!state;
                const settings = options || {};
            ...
            const handleDocumentPointer = function(event) {
                if (!isOpen) {
                    return;
                }
            ...
            const initialDismissed = readDismissedPreference();
